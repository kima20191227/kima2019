...
Fix:
- Added `/api/upload/resource/signed` to issue Supabase Storage signed upload URLs after auth, MIME, and size validation.
- Updated `uploadResourceFile()` so browser uploads go directly to Supabase Storage via the signed URL.
- Kept the old server upload route only as a small-file fallback.
...
- `npm run build` passed and included `/api/upload/resource/signed`.

Status: DONE_WITH_CONCERNS

Remaining verification:
- Production should be tested with an authenticated 20MB file upload after deployment, because local tests cannot exercise the deployed platform body limit and Supabase Storage CORS behavior.
