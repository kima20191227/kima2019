...
> 이 파일은 VS Code Codex가 항상 참조하는 프로젝트 컨텍스트입니다.
...
```
이 AGENTS.md 파일을 읽고 프로젝트를 초기화해줘.
...
9. .env.example 파일 생성 (AGENTS.md의 환경변수 섹션 참고, 값은 비워둠)

10. AGENTS.md의 디렉토리 구조대로 폴더와 빈 파일 생성
    (schemas/ 폴더와 tests/ 폴더 포함)

11. prisma/schema.prisma 파일을 AGENTS.md의 스키마로 작성
...
```
AGENTS.md를 참고해서 Supabase와 Prisma를 설정해줘.
...
```
AGENTS.md의 schemas/ 폴더 구조를 참고해서 Zod 검증 스키마를 작성해줘.
...
```
AGENTS.md를 참고해서 NextAuth.js로 이메일 인증을 구현해줘.
...
```
AGENTS.md의 디자인 가이드를 참고해서 공통 레이아웃을 만들어줘.
...
```
AGENTS.md를 참고해서 전국 사역 단체 디렉토리와 지도를 구현해줘.
...
```
AGENTS.md를 참고해서 3축 카테고리 게시판을 구현해줘.
...
```
AGENTS.md의 이메일 자동화 명세를 참고해서 이메일 발송을 구현해줘.
...
```
AGENTS.md를 참고해서 핵심 기능에 대한 테스트 코드를 작성해줘.
...
```
AGENTS.md를 참고해서 SEO, 보안, 성능 최적화를 적용해줘.
